...
def get_input_url():
    """
    Lấy URL theo thứ tự:
    1. Biến môi trường OCTO_URL - dùng cho GitHub Actions.
    2. Tham số dòng lệnh - dùng khi chạy máy cá nhân.
    """
...
def get_proxy_config():
    """
    Đọc cấu hình proxy tương thích tuyệt đối với GitHub Actions (PROXY_SERVER, PROXY_USER, PROXY_PASS)
    hoặc PROXY_URL truyền thống.
    """
...
def load_ignored_domains():
    """Đọc file domains.txt để nhận biết các domain trung gian cần bỏ qua."""
    ignored = set()
...
def click_next_button(page):
    """
    Tìm và click nút chuyển tiếp nếu trang có nút.
    Chỉ xử lý nút hiển thị trên trang.
    """
...
def get_visible_links(page):
    """
    Lấy các link HTTP/HTTPS đang có trong HTML.
    """
    try:
    ...
    for link in links:
        if is_http_url(link) and link not in result:
            result.append(link)
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            for step in range(20):
                ...
                for link in links:
                    if link != target_url:
                        print(f"[*] Phát hiện link trong trang: {link}")
