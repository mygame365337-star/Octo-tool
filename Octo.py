...
def bypass_octolink_sequential(target_url):
    ...
    # Định nghĩa thời gian chờ cho từng step (bạn có thể tăng giảm tùy ý)
    steps = [
        (1, 3), # Step 1
        (2, 3), # Step 2
        (3, 3), # Step 3
    ...
    for step_num, delay in steps:
        ...
        # Gửi request lên API theo từng step tuần tự
        ...
        if job_res.status_code == 200:
            ...
            # Ở Step cuối cùng (Step 4), server sẽ trả về gói JSON chứa link đích
            if step_num == 4:
                try:
                    data = job_res.json()
                    if "url" in data:
    ...
    # Nếu server không trả về URL trực tiếp trong JSON, tạo link finish từ token
    if not final_url:
...
if __name__ == "__main__":
    # Nhận link truyền vào hoặc mặc định dùng link mẫu của bạn
    url = os.getenv("OCTO_URL", "https://octolink.vip/vwUe")
